...
.PHONY: bootstrap lint format typecheck test build verify repro-check gate-v37-plan gate-federation-doctrine gate-a01 gate-a02 gate-a03 gate-a04 gate-a05 gate-a06 gate-a07 gate-a08 gate-a09 gate-a10 gate-a11 gate-a12 gate-a13 gate-a14 gate-a15 gate-a16 gate-a17 gate-a18 gate-a19 gate-a20 gate-a21 gate-a22 gate-wp03 gate-wp10 gate-wp11 gate-wp12 gate-wp13 gate-wp14 gate-wp45 corpus router-determinism clean help
...
gate-v37-plan: ## Run the V3.7 plan mutable-state consistency gate.
	uv run python scripts/checks/srf-v37-plan-consistency.py

gate-federation-doctrine: ## Verify static federation ownership and mandatory role entrypoints.
...
gate-a10: ## Run the V3.7 A10 independent prover activation gate.
	uv run python scripts/checks/srf-v37-a10-gate.py

gate-a11: ## Run the V3.7 A11 knowledge graph activation gate.
	uv run python scripts/checks/srf-v37-a11-gate.py

gate-a12: ## Run the V3.7 A12 discovery and dynamics activation gate.
	uv run python scripts/checks/srf-v37-a12-gate.py
...
gate-a16: ## Run the V3.7 A16 scientific products activation gate.
	uv run python scripts/checks/srf-v37-a16-gate.py

gate-a17: ## Run the V3.7 A17 solo-agent entry gate.
	uv run python scripts/checks/srf-v37-a17-gate.py

gate-a18: ## Run the V3.7 A18 DualContour native child closeout gate.
	uv run python scripts/checks/srf-v37-a18-gate.py

gate-a19: ## Run the V3.7 A19 Market native bridge closeout gate.
	uv run python scripts/checks/srf-v37-a19-gate.py

gate-a20: ## Run the V3.7 A20 Security native bridge closeout gate.
	uv run python scripts/checks/srf-v37-a20-gate.py

gate-a21: ## Run the V3.7 A21 disaster recovery and chaos gate.
	uv run python scripts/checks/srf-v37-a21-gate.py

gate-a22: ## Run the V3.7 A22 final acceptance and false-closure gate.
	uv run python scripts/checks/srf-v37-a22-gate.py
